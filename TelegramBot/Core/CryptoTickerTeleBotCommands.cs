...
namespace TelegramBot.Core
{
	public partial class TeleBot
	{
		...
		private async Task HandleCompare ( Message message, IList<string> @params )
		{
			...
			if ( @params?.Count >= 2 )
				compare = Ctb.CompareTable.GetCompare (
					@params
						.Where ( x => GetExchangeBase ( x ) != null )
				...
				compare = Ctb.CompareTable.GetAllCompare ( );
		...
		private async Task HandleBest ( Message message, IList<string> @params )
		{
            ...
            {
                var param = GetExchangeBase(@params[0]);
                await HandleBestAllBetweenExchanges(message, param).ConfigureAwait(false);
                return;
            }
            ...
            if ( @params == null || @params.Count < 2 )
			{
                ...
                await HandleBestAllBetweenExchanges(message).ConfigureAwait(false);
                return;
			}
